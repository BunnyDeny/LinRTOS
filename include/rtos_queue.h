...
struct rtos_queue {
    ...
    /* ============================================================
     * 🔐 互斥锁专用字段（仅在 queue_type == MUTEX / RECURSIVE 时使用）
     * ============================================================ */
    struct rtos_tcb *mutex_holder;   /* 当前持有者；NULL = 无 */
    uint32_t recursive_count;        /* 递归计数（递归互斥锁用） */
    uint32_t original_priority;      /* 持有者被提升前的原始优先级；
                                        0xFFFFFFFF = 未提升 */
};
