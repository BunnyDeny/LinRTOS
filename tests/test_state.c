...
static void task_query(void *param)
{
    ...
    {
        RTOS_ENTER_CRITICAL();
        debug_printf("[QRY ] self  state=%d prio=%lu me=%p\r\n",
    ...
    {
        RTOS_ENTER_CRITICAL();
        debug_printf("[QRY ] helper state=%d (expected BLOCKED=2)\r\n",
                     (int)helper_state);
        RTOS_EXIT_CRITICAL();
    }
    ...
    {
        RTOS_ENTER_CRITICAL();
        debug_printf("[QRY ] helper state=%d after 500 ticks\r\n",
                     (int)helper_state);
        RTOS_EXIT_CRITICAL();
    }
    ...
    {
        RTOS_ENTER_CRITICAL();
        debug_printf("[QRY ] test done, entering idle\r\n");
        RTOS_EXIT_CRITICAL();
    }
    for (;;) {
