/*
 * Test: Dynamic priority change + task state query
 *
 * 验证项：
 *  - rtos_task_set_priority 动态调整优先级
 *  - rtos_task_get_priority 查询优先级
 *  - rtos_task_get_state    查询状态
...
static void task_worker(void *param)
{
    (void)param;
    for (;;) {
        rtos_task_state_t state = rtos_task_get_state(NULL);
        uint32_t prio = rtos_task_get_priority(NULL);
        rtos_task_handle_t me = rtos_task_get_current();

        debug_printf("[WORK] tick=%lu prio=%lu state=%d me=%p\r\n",
